...
//            legend.setPosition(LegendPosition.TOP_RIGHT);
            ...
            //数据1，
            XDDFNumericalDataSource<Double> values = XDDFDataSourcesFactory.fromNumericCellRange(sheet, new CellRangeAddress(2, 2, 0, 6));
            ...
            dLbls.setSeparator("\n");//分隔符为分行符
//			dLbls.addNewDLblPos().setVal(Enum.forString("inEnd"));//数据标签内

            // 将输出写入excel文件
            fileOut = new FileOutputStream("E:\\excel-chart-" + System.currentTimeMillis() + ".xlsx");
            wb.write(fileOut);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
